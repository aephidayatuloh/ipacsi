...
# launch <- function(){
  ...
  server <- function(input, output, session){
    ...
    ic <- eventReactive(input$submit, {
      if(is.null(input$data)){
        ...
        confirmSweetAlert(
          ...
          type = "error",
          title = "Ooops",
          text = "There is no data uploaded",
        ...
        confirmSweetAlert(
          session = session,
          inputId = "sheet", type = "error",
          title = "Ooops",
          text = "Performance and Importance sheet name can not be the same.",
          showCloseButton = FALSE
        )
        return(NULL)
      } else {
        ...
        confirmSweetAlert(
          session = session,
          inputId = "sheetwait", type = "info",
          title = "Please wait...",
          text = "Your request will be done in minutes", btn_labels = "OK", btn_colors = "#04B404",
    ...
    observeEvent(input$submit, {
      output$quadrant <- renderUI({
        if(!is.null(ic())){
          ...
          confirmSweetAlert(
            session = session,
            inputId = "sheetwait", type = "success",
            title = "Done!",
            text = "", showCloseButton = FALSE,
            btn_labels = "OK", btn_colors = "#04B404",
            closeOnClickOutside = TRUE
          )
          )
        } else {
    ...
    observeEvent(input$faq, {
      ...
      #            type = "", html = TRUE
      # )
      confirmSweetAlert(ssession,
                        inputId = "faq",
                        title = "Get Started",type = "info", showCloseButton = FALSE, btn_labels = "Close", html = TRUE,
                        text = "<ul style = 'text-align:left;list-style-type:disc;'>
                               <li>Input file must an Excel (*.xls or *.xlsx) file.</li>
                               <li>At least consist of two sheets.</li>
                               <li>Each sheet represent data for performance and importance survey result.</li>
